...
func solve(input []string) {
	...
	wonTimes := make([]int, len(input))
	for i := range wonTimes {
		wonTimes[i] = 1
	}
	...
	for index, line := range input {
		...
		fmt.Println("Winning number ", index, "wonTimes", wonTimes[index], ": ", winningNumbers)
		for i := 0; i < len(winningNumbers); i++ {
			if i+index+1 < len(wonTimes) {
				wonTimes[i+index+1] += wonTimes[index]
			}
		}
	}
	fmt.Println("Sum: ", sum)
